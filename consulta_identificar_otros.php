...
require("menu.php");
?>
<p class="titulo_tab_principal">Buscador de Otros</p>
...
if (isset($_POST["coincidencia"]) or isset($_POST["rango"]))
	{
		...
		if	($condicion != "Error")
			{
				$sql="SELECT * FROM otros WHERE $condicion AND estado ='A'";
				...
				while($fila=mysql_fetch_array($consulta))
					{
						...
						$hora=$fila["hora"];
						$id_otros=$fila["id"];
						$observaciones=substr($fila["observaciones"],0,30)."...";
						
						//Discriminacion de aerolinea de usuario TIPO 6						
						$sql2="SELECT g.id_aerolinea,c.* FROM guia g LEFT JOIN carga_otros c ON g.id=c.id_guia WHERE c.id_otros='$id_otros'";
							...
							if($id_aerolinea_user == $id_aerolinea)
							{
								echo "<tr>
										<td align=\"center\" class=\"celda_tabla_principal celda_boton\">
											<button type=\"button\" onClick=\"document.location='despacho_otros3.php?id=$id_otros'\">
												<img src=\"imagenes/buscar-act.png\" title=\"Consulta\" />
											</button>
										</td>
										<td align=\"center\" class=\"celda_tabla_principal celda_boton\">
											<button type=\"button\" onClick=\"document.location='modificar_otros1.php?id=$id_otros'\" $activacion1>
												<img src=\"imagenes/settings-act.png\" title=\"Modificar\" />
											</button>
										</td>
										<td align=\"center\" class=\"celda_tabla_principal celda_boton\">
											<button type=\"button\" onClick=\"conf_eliminar($id_otros);\" $activacion2>
												<img src=\"imagenes/eliminar-act.png\" title=\"Eliminar\" />
											</button>
										</td>								
										<td class=\"celda_tabla_principal celda_boton\"> $id_otros </td>
										<td class=\"celda_tabla_principal celda_boton\"> $fecha </td>
										<td class=\"celda_tabla_principal celda_boton\"> $hora </td>
										<td class=\"celda_tabla_principal celda_boton\"> $observaciones </td>
								</tr>";							
							}
						}
						else
