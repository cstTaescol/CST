...
if(isset($_REQUEST["id_vuelo"]))
	{
		...
		$hora_estimada=$_REQUEST["hora_estimada"];
...
require("menu.php");
?>
<p class="titulo_tab_principal">Modificacion de Vuelo</p>
<form name="ingresovuelo" method="post" action="vuelo_modificar_guias2.php">
  ...
  <tr>
    ...
    <td class="celda_tabla_principal"><div class="letreros_tabla">Destino</div></td>
   	<td class="celda_tabla_principal">
    	<button type="button" onclick="seleccionar();"><img src="imagenes/aceptar-act.png" height="33" width="29" title="Seleccionar Todos" /></button>
        <button type="button" onclick="deseleccionar();"><img src="imagenes/aceptar-in.png" height="33" width="29" title="Quitar todas las Selecciones" /></button>   		
   </td>
  </tr>
<?php
$sql="SELECT master,hija,piezas,peso,volumen,id,id_deposito,id_tipo_bloqueo,id_tipo_guia,id_disposicion,id_vuelo FROM guia WHERE id_aerolinea = '$id_aerolinea' AND id_tipo_guia != '2' AND (id_tipo_bloqueo='7' OR id_tipo_bloqueo='1') AND (id_vuelo='$id_vuelo' OR ISNULL(id_vuelo) OR id_vuelo='') ORDER BY master, id_deposito ASC";
$consulta=mysql_query ($sql,$conexion) or die ("ERROR: ". mysql_error(). " INFORME AL SOPORTE TECNICO");
...
if ($nguias > 0)
{
	for ($i=1; $i<=$nguias; $i++)
	{
		...
		$id_guia=$fila["id"];
		$id_estevuelo=$fila["id_vuelo"];
		if ($id_estevuelo != NULL)
		{
			$accion='checked="checked"';
			$valor=1; //cuando Ya esta asignada a un vuelo
		}
			else
			{
				$accion="";
				$valor=0; //cuando No esta asignada a un vuelo
			}
		...
		echo '<tr>
				...
				<td align="center" class="celda_tabla_principal celda_boton">
					<input type="checkbox" name="chkacepto'.$i.'" id="chkacepto'.$i.'" value="1" '.$accion.'/>
...
<table width="450" align="center">
    ...
    <tr>
      <td align="center" valign="middle" class="celda_tabla_principal celda_boton">
        ...
        <button type="submit" name="guardar" id="guardar" > <img src="imagenes/guardar-act.png" alt="" title="Guardar" /> </button>
      </td>
    </tr>
</table>
<?php 
