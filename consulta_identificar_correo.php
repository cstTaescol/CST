...
if (respuesta)
	{
		window.location="eliminar_correo1.php?id="+url;
	}
}
</script>
</head>
...
require("menu.php");
?>
<p class="titulo_tab_principal">Buscador de Guias de Correo</p>
...
if (isset($_POST["coincidencia"]) or isset($_POST["rango"]))
	{
		...
		if	($condicion != "Error")
			{
				$sql="SELECT * FROM correo WHERE $condicion AND estado ='A' $sql_aerolinea";
				...
				while($fila=mysql_fetch_array($consulta))
					{
						...
						echo "<tr>
								<td align=\"center\" class=\"celda_tabla_principal celda_boton\">
									<button type=\"button\" onClick=\"abrir('pdf_correo.php?id=$id_correo')\">
										<img src=\"imagenes/imprimir-act.png\" title=\"Impresion\" />									
									</button>
								</td>
								<td align=\"center\" class=\"celda_tabla_principal celda_boton\">
									<button type=\"button\" onClick=\"document.location='modificar_correo1.php?id=$id_correo'\" $activacion1>
										<img src=\"imagenes/settings-act.png\" title=\"Modificar\" />									
									</button>
								</td>
								<td align=\"center\" class=\"celda_tabla_principal celda_boton\">
									<button type=\"button\" onClick=\"conf_eliminar($id_correo);\" $activacion2>
										<img src=\"imagenes/eliminar-act.png\" title=\"Eliminar\" />									
									</button>
								</td>																
								<td class=\"celda_tabla_principal celda_boton\"> $id_correo </td>
