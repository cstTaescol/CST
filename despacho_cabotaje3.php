...
{	
	...
	$sql="INSERT INTO cabotaje (id_guia,
							  destinatario,
							  id_usuario,
							  fecha,
							  hora,
	...
	$consulta_posicion=mysql_query ($sql_posiscion,$conexion) or die (exit('Error al encontrar posiciones a Liberar '.mysql_error()));
	while($fila_posicion=mysql_fetch_array($consulta_posicion))
	{
		...
		mysql_query ($sql,$conexion) or die (exit("0-5-0- "));
	}
	...
	//***********************************
	echo $msg_exit = "1-0-$id_despacho-0";
}
else
	echo $msg_exit = "0-6-0- ";
?>