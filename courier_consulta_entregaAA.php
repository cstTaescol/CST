...
<?php
	require("menu.php");
?>
<p class="titulo_tab_principal">Buscador Entregas A.Aduanera</p>
...
if (isset($_POST["coincidencia"]) or isset($_POST["rango"]))
	{
		...
		if	($condicion != "Error")
			{				
				$sql="SELECT id,piezas,peso,id_guia,fecha,hora FROM courier_despacho_hija WHERE $condicion ORDER BY fecha DESC";
				...
				while($fila=mysql_fetch_array($consulta))
					{						
						$piezas=$fila["piezas"];	
						//REVISAR DECIMALES DEL PESO NO APROXIMNADOS A 0					
						...
						$sql3="SELECT hija,master,id_consignatario FROM guia  WHERE id ='$id_guia'";
						$consulta3=mysql_query ($sql3,$conexion) or die ("ERROR 2.1: ". mysql_error(). " INFORME AL SOPORTE TECNICO");
						$fila3=mysql_fetch_array($consulta3);
						$hija=$fila3["hija"];
						$master=$fila3["master"];
						
						$sql3="SELECT id_consignatario FROM guia  WHERE id ='$master'";
						$consulta3=mysql_query ($sql3,$conexion) or die ("ERROR 2.2: ". mysql_error(). " INFORME AL SOPORTE TECNICO");
						$fila3=mysql_fetch_array($consulta3);
						...
						echo '<tr>
								<td align="center" class="celda_tabla_principal celda_boton">
									<button type="button" onClick="document.location=\'courier_despacho_opciones_hija.php?id_registro='.$id_registro.'\'">
